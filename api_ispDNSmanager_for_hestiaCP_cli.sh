...
add_domen_record_to_DNSmanager(){
    query=$(curl -s -X POST -d "auth=$key&out=json&lang=ua&func=domain.record.edit&plid=$domen&elid=$sub_domen&name=$sub_domen&rtype=$type&ip=$ip_domen&value=$value" "$URL_DNS_MANAGER")
    echo "$query"
}
...
domain_record_edit(){
    ...
    echo -e "$query\n\n\n\n"
    query=$(curl -s -X POST -d "auth=$key&out=json&lang=ua&func=domain.record.edit&tconvert=punycode&plid=$domen&elid=$domen&sok=ok" "$URL_DNS_MANAGER")
    echo "$query"
}
